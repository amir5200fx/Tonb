...
namespace tnbLib
{
	...
	) const
	{
		...
		forAll(slavePatch_, faceSi)
		{
			...
			scalar tmpValue = 0.25*tnbLib::magSqr(bbSlave.max() - bbSlave.min());
			...
			//     forAll (masterPatch_, faceMi)
		{
			...
			scalar masterRadius2 =
				tnbLib::magSqr(bbMaster.max() - bbMaster.min()) / 4.0;
	...
	) const
	{
		...
		forAll(slaveFaceBBminThickness, sI)
		{
			...
			forAll(el, elI)
			{
				...
				maxEdgeLength = tnbLib::max(edgeLength, maxEdgeLength);
			}
	...
	) const
	{
				...
				)
		{
			...
			scalar p1_min = tnbLib::min(poly1PointsProjection);
			scalar p1_max = tnbLib::max(poly1PointsProjection);
			scalar p2_min = tnbLib::min(poly2PointsProjection);
			scalar p2_max = tnbLib::max(poly2PointsProjection);
