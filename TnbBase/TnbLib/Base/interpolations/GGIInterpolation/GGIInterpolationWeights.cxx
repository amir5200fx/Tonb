...
namespace tnbLib
{
	...
	void GGIInterpolation<MasterPatch, SlavePatch>::rescaleWeightingFactors() const
	{
		...
		forAll(saW, saWi)
		{
			scalar slaveWeightSum = tnbLib::sum(saW[saWi]);
			...
			if (saW[saWi].size() > 0)
			{
				...
				largestSWC = tnbLib::max(largestSWC, curSWC);
		...
		forAll(maW, maWi)
		{
			scalar masterWeightSum = tnbLib::sum(maW[maWi]);
			...
			if (maW[maWi].size() > 0)
			{
				...
				largestMWC = tnbLib::max(largestMWC, curMWC);
		...
		calcMasterPointAddressing() const
	{
		...
		forAll(masterPointAddr, pointI)
		{
			...
			forAll(possibleSlaveFaces, faceI)
			{
				...
				point ctr = tnbLib::average(f.points(slavePoints));
		...
		calcMasterPointWeights() const
	{
		...
		forAll(masterPointWeights, pointI)
		{
			if (addr[pointI].first() != -1)
			{
				...
				point ctr = tnbLib::average(hitFace.points(slavePoints));
		...
		calcSlavePointAddressing() const
	{
		...
		forAll(slavePointAddr, pointI)
		{
			...
			forAll(possibleMasterFaces, faceI)
			{
				...
				point ctr = tnbLib::average(f.points(masterPoints));
		...
		calcSlavePointWeights() const
	{
		...
		forAll(slavePointWeights, pointI)
		{
			if (addr[pointI].first() != -1)
			{
				...
				point ctr = tnbLib::average(hitFace.points(masterPoints));
