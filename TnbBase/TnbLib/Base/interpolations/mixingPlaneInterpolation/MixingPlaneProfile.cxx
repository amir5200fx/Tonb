...
namespace tnbLib
{
		...
		MixingPlaneInterpolation<MasterPatch, SlavePatch>::calcProfile() const
	{
		...
		scalar halfMinSizeBin =
			tnbLib::max(masterMinEdgeLength, slaveMinEdgeLength) / 2.0;
		...
		boundBox globSpanBB
		(
			point(tnbLib::min(masterBB.min(), slaveBB.min()))
			- point(SMALL, SMALL, SMALL),
			point(tnbLib::max(masterBB.max(), slaveBB.max()))
			+ point(SMALL, SMALL, SMALL)
		);
