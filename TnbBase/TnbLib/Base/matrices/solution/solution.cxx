...
tnbLib::solution::solution(const objectRegistry& obr, const fileName& dictName)
	:
	IOdictionary
	(
		IOobject
	...
	prevTimeIndex_(0)
{
	if (!headerOk())
	{
		if (debug)
		{
			InfoIn
			(
				"tnbLib::solution::solution(const objectRegistry& obr, "
...
tnbLib::scalar tnbLib::solution::fieldRelaxationFactor(const word& name) const
{
	...
	else
	{
		FatalIOErrorIn
		(
			"tnbLib::solution::fieldRelaxationFactor(const word&)",
			fieldRelaxDict_
...
tnbLib::scalar tnbLib::solution::equationRelaxationFactor(const word& name) const
{
	...
	else
	{
		FatalIOErrorIn
		(
			"tnbLib::solution::eqnRelaxationFactor(const word&)",
			eqnRelaxDict_
...
const tnbLib::dictionary& tnbLib::solution::solver(const word& name) const
{
	if (debug)
	{
...
bool tnbLib::solution::writeData(Ostream& os) const
{
...
tnbLib::dictionary& tnbLib::solution::solverPerformanceDict() const
{
	return solverPerformance_;
}


void tnbLib::solution::setSolverPerformance
(
	const word& name,
