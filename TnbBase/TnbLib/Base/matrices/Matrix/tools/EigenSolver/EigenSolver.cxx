...
template<class T>
void tnbLib::EigenSolver<T>::checkMatrix(const SquareMatrix<T>& mtx) const
{
	// Check symmetry
...
template<class T>
void tnbLib::EigenSolver<T>::factorise(const SquareMatrix<T>& mtx)
{
			...
			for (label iq = ip + 1; iq < n; iq++)
			{
				...
				else if (mag(a[ip][iq]) > treshold)
				{
					...
					else
					{
						...
						t = 1.0 / (mag(theta) + tnbLib::sqrt(1.0 + sqr(theta)));
...
template<class T>
inline void tnbLib::EigenSolver<T>::rotate
(
	SquareMatrix<T>& a,
	const T s,
	const T tau,
...
template<class T>
tnbLib::EigenSolver<T>::EigenSolver(const SquareMatrix<T>& mtx)
	:
...
template<class T>
T tnbLib::EigenSolver<T>::eigenValue(const label n) const
{
	return values_[n];
}
...
template<class T>
const tnbLib::List<T>& tnbLib::EigenSolver<T>::eigenVector(const label n) const
{
	return vectors_[n];
}
