...
defineTypeNameAndDebug(tnbLib::MSwindows, 0);
...
namespace tnbLib
{
	...
	namespace MSwindows
	{
		...
		//- tnbLib's file io (see src/OpenFOAM/db/IOstreams/Sstreams/OSwrite.C)
	...
	{
		// Not supported but assume worst case for tnbLib::dynamicCode::checkSecurity
		return true;
	}
	...
	fileName findEtcFile(const fileName& name, bool mandatory)
	{
		...
		if (isDir(searchDir))
		{
			// Check for shipped TONB file in $WM_PROJECT_DIR/etc
			fileName fullName = searchDir / "etc" / name;
			if (isFile(fullName))
			{
				return fullName;
		...
		if (isDir(searchDir))
		{
			// Check for user file in ~/.TONB/VERSION
			fileName fullName = searchDir / TONBversion / name;
			if (isFile(fullName))
			{
				return fullName;
			}

			// Check for version-independent user file in ~/.TONB
			fullName = searchDir / name;
			if (isFile(fullName))
			{
				return fullName;
		...
		if (isDir(searchDir))
		{
			...
			fileName fullName = searchDir / "site" / TONBversion / name;
			if (isFile(fullName))
			{
				return fullName;
			}
		...
		if (mandatory)
		{
			cerr << "--> FOAM FATAL ERROR in tnbLib::findEtcFile() :"
				" could not find mandatory file\n    '"
				<< name.c_str() << "'\n\n" << std::endl;
			::exit(1);
		}
