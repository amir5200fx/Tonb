...
tnbLib::tmp
<
	typename tnbLib::GeometricField<Type, PatchField, GeoMesh>::
	GeometricBoundaryField
>
tnbLib::GeometricField<Type, PatchField, GeoMesh>::readField
(
...
<
	...
	GeometricBoundaryField
>
tnbLib::GeometricField<Type, PatchField, GeoMesh>::readField(Istream& is)
{
	if (is.version() < 2.0)
	{
		FatalIOErrorIn
...
tnbLib::GeometricField<Type, PatchField, GeoMesh>::boundaryField()
{
	this->setUpToDate();
	storeOldTimes();
...
tnbLib::GeometricField<Type, PatchField, GeoMesh>::T() const
{
	...
	tnbLib::T(result().internalField(), internalField());
	tnbLib::T(result().boundaryField(), boundaryField());
...
) const
{
	...
	tnbLib::component(Component().internalField(), internalField(), d);
	tnbLib::component(Component().boundaryField(), boundaryField(), d);
...
{
	tnbLib::max(internalField(), internalField(), dt.value());
	tnbLib::max(boundaryField(), boundaryField(), dt.value());
}
...
{
	tnbLib::min(internalField(), internalField(), dt.value());
	tnbLib::min(boundaryField(), boundaryField(), dt.value());
}
