...
tnbLib::label tnbLib::cyclicPolyPatch::findMaxArea
(
...
void tnbLib::cyclicPolyPatch::calcTransforms() const
{
	if (size())
	{
		...
		for (label facei = 0; facei < size() / 2; facei++)
		{
					...
					)
			{
				...
				maxMatchError =
					tnbLib::max(maxMatchError, mag(magSf - nbrMagSf) / avSf);
				errorFace = facei;
			}
			else
			{
			...
			if (reverseT_.size() > 0)
			{
				...
				forAll(transformPoints, pointI)
				{
					transformPoints[pointI] =
						tnbLib::transform(reverseT_[0], transformPoints[pointI]);
				}
				...
				if (reverseT_.size())
				{
					maxDistance =
						...
						(
							...
							mag
							(
								tnbLib::transform(reverseT_[0], half0Ctrs[faceI])
								- half1Ctrs[faceI]
							)
						);
...
bool tnbLib::cyclicPolyPatch::getGeometricHalves
(
	...
	if (debug)
	{
		...
		for (label zoneI = 0; zoneI < ppZones.nZones(); zoneI++)
		{
			...
			(
				...
				/ name() + "_zone_" + tnbLib::name(zoneI) + ".obj"
			);
...
// right ones.
void tnbLib::cyclicPolyPatch::getCentresAndAnchors
(
	const primitivePatch& pp,
...
) const
{
	...
	if (mag(n0 & n1) < 1 - polyPatch::matchTol_())
	{
		...
		forAll(half0Ctrs, faceI)
		{
			half0Ctrs[faceI] = tnbLib::transform(reverseT, half0Ctrs[faceI]);
			anchors0[faceI] = tnbLib::transform(reverseT, anchors0[faceI]);
		}
	}
	else
	{
...
bool tnbLib::cyclicPolyPatch::matchAnchors
(
	const bool report,
	const primitivePatch& pp,
...
tnbLib::label tnbLib::cyclicPolyPatch::getConsistentRotationFace
(
...
tnbLib::cyclicPolyPatch::cyclicPolyPatch
(
	const word& name,
	const label size,
	const label start,
...
void tnbLib::cyclicPolyPatch::initAddressing()
{
	polyPatch::initAddressing();
}


void tnbLib::cyclicPolyPatch::calcAddressing()
{
	polyPatch::calcAddressing();
}


void tnbLib::cyclicPolyPatch::initGeometry()
{
	polyPatch::initGeometry();
}


void tnbLib::cyclicPolyPatch::calcGeometry()
{
...
void tnbLib::cyclicPolyPatch::initMovePoints(const pointField& p)
{
	polyPatch::initMovePoints(p);
}


void tnbLib::cyclicPolyPatch::movePoints(const pointField& p)
{
...
void tnbLib::cyclicPolyPatch::initUpdateMesh()
{
	polyPatch::initUpdateMesh();
}


void tnbLib::cyclicPolyPatch::updateMesh()
{
...
const tnbLib::edgeList& tnbLib::cyclicPolyPatch::coupledPoints() const
{
...
const tnbLib::edgeList& tnbLib::cyclicPolyPatch::coupledEdges() const
{
	if (!coupledEdgesPtr_)
	{
...
void tnbLib::cyclicPolyPatch::initOrder(const primitivePatch& pp) const
{}
...
bool tnbLib::cyclicPolyPatch::order
(
	const primitivePatch& pp,
	labelList& faceMap,
	labelList& rotation
...
void tnbLib::cyclicPolyPatch::syncOrder() const
{}


void tnbLib::cyclicPolyPatch::write(Ostream& os) const
{
	polyPatch::write(os);
