...
tnbLib::SLList<tnbLib::string> tnbLib::argList::validArgs;
tnbLib::HashTable<tnbLib::string> tnbLib::argList::validOptions;
tnbLib::HashTable<tnbLib::string> tnbLib::argList::validParOptions;
tnbLib::word tnbLib::argList::appDictName_("");
bool tnbLib::argList::bannerEnabled(true);


tnbLib::argList::initValidTables::initValidTables()
{
...
tnbLib::argList::initValidTables dummyInitValidTables;


// convert argv -> args_
// transform sequences with "(" ... ")" into string lists in the process
bool tnbLib::argList::regroupArgv(int& argc, char**& argv)
{
	int nArgs = 0;
	int listDepth = 0;
	string tmpString;
...
//
void tnbLib::argList::getRootCase()
{
	fileName casePath;
...
tnbLib::stringList::subList tnbLib::argList::additionalArgs() const
{
	...
	options_(argc)
{
	...
	jobInfo.add("foamVersion", word(TONBversion));
	jobInfo.add("foamBuild", tnbLib::TONBbuild);
...
void tnbLib::argList::noBanner()
{
	bannerEnabled = false;
}


void tnbLib::argList::noParallel()
{
	validOptions.erase("parallel");
}


void tnbLib::argList::printUsage() const
{
	Info << nl
		<< "Usage: " << executable_;
	...
	int i = 0;
	SortableList<tnbLib::string> sortedValidOptions(validOptions.size());
...
void tnbLib::argList::displayDoc(bool source) const
{
...
bool tnbLib::argList::check(bool checkArgs, bool checkOpts) const
{
	bool ok = true;
...
bool tnbLib::argList::checkRootCase() const
{
	if (!isDir(rootPath()))
	{
		FatalError
