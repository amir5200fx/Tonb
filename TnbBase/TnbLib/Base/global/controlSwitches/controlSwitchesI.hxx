...
namespace tnbLib
{
	...
	namespace debug
	{
		...
		template<class T>
		controlSwitches<T>::controlSwitches
		(
		...
		)
		{
			...
			if (mapListSwitchesPtr)
			{
				...
				for (it = mapListSwitches.begin(); it != mapListSwitches.end(); it++)
				{
					...
					std::list<controlSwitches<T> *> swList = it->second;
					controlSwitches<T>& value = *(swList.front());
					...
					typename
						std::list<controlSwitches<T> *>::iterator itL;
