...
# endif
					...
					localMinArea = tnbLib::min(localMinArea, magFaceArea);
					localMaxArea = tnbLib::max(localMaxArea, magFaceArea);
				}
...
# endif
				{
					minArea = tnbLib::min(minArea, localMinArea);
					maxArea = tnbLib::max(maxArea, localMaxArea);
				}
			}
...
# endif
					...
					localMinDDotS = tnbLib::min(dDotS, localMinDDotS);
					sumDDotS += dDotS;
				}
...
# pragma omp critical
# endif
				minDDotS = tnbLib::min(minDDotS, localMinDDotS);
			}
...
# endif
					...
					localMaxSkew = tnbLib::max(localMaxSkew, skewness);
					sumSkew += skewness;
				}
...
# pragma omp critical
# endif
				maxSkew = tnbLib::max(maxSkew, localMaxSkew);
			}
...
# endif
			for (label faceI = 0; faceI < nInternalFaces; ++faceI)
			{
				...
				const scalar dOwn
				(
					tnbLib::mag(centres[owner[faceI]] - fCentres[faceI])
				);
				const scalar dNei
				(
					tnbLib::mag(centres[neighbour[faceI]] - fCentres[faceI])
				);

				faceUniformity[faceI] = tnbLib::min(dOwn, dNei) / (dOwn + dNei);
			}
			...
			if (Pstream::parRun())
			{
				...
				forAll(procBoundaries, patchI)
				{
					...
					forAll(dst, faceI)
					{
						...
						dst[faceI] = tnbLib::mag(centres[owner[fI]] - fCentres[fI]);
					}
				...
				forAll(procBoundaries, patchI)
				{
					...
					forAll(otherDst, faceI)
					{
						const label fI = start + faceI;
						const scalar dOwn =
							tnbLib::mag(centres[owner[fI]] - fCentres[fI]);
						const scalar dNei = otherDst[faceI];
						faceUniformity[fI] = tnbLib::min(dOwn, dNei) / (dOwn + dNei);
					}
				}
			}
		}
...
# endif
					...
					localMaxUniformity = tnbLib::max(localMaxUniformity, uniformity);
					localMinUniformity = tnbLib::min(localMinUniformity, uniformity);
...
# endif
				{
					maxUniformity = tnbLib::max(maxUniformity, localMaxUniformity);
					minUniformity = tnbLib::min(minUniformity, localMinUniformity);
				}
			}
			...
			if (Pstream::parRun())
			{
				...
				forAll(procBoundaries, patchI)
				{
					...
					for (label fI = 0; fI < size; ++fI)
					{
						...
						maxUniformity = tnbLib::max(maxUniformity, uniformity);
						minUniformity = tnbLib::min(minUniformity, uniformity);
		...
		)
		{
			...
			const scalar maxSin = tnbLib::sin(maxDeg / 180.0*M_PI);
...
# endif
									{
										...
										localMaxEdgeSin =
											tnbLib::max(localMaxEdgeSin, magEdgeNormal);
									}
								}
							}
						}
...
# pragma omp critical
# endif
				maxEdgeSin = tnbLib::max(maxEdgeSin, localMaxEdgeSin);
			}
...
# endif
				forAll(faces, faceI)
				{
					...
					if (f.size() > 3 && magAreas[faceI] > VSMALL)
					{
						...
						minFlatnessProc = tnbLib::min(minFlatnessProc, flatness);
...
# endif
				{
					minFlatness = tnbLib::min(minFlatness, minFlatnessProc);
				}
			}
		...
		)
		{
			...
			Info << "Worst non-orthogonality " << tnbLib::acos(minNonOrtho) * 180.0 / M_PI
				<< " selecting faces with non-orthogonality greater than "
				<< (tnbLib::acos(warnNonOrtho) * 180.0 / M_PI) << endl;
