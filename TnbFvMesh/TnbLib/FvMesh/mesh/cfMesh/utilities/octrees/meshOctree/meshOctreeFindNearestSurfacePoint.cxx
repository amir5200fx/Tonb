		...
		do
		{
			...
			forAll(neighbours, neiI)
			{
				...
				forAll(el, tI)
				{
					...
					const scalar dSq = tnbLib::magSqr(p0 - p);
					if (dSq < distSq)
					{
						distSq = dSq;
						nearest = p0;
...
# endif
		...
		do
		{
			...
			forAll(neighbours, neiI)
			{
				...
				forAll(ce, eI)
				{
					...
					const scalar dSq = tnbLib::magSqr(np - p);
					...
					forAll(tri, pI)
					{
						...
						if (nEdges > 2)
						{
							...
							if (nEdges >= patches.size())
							{
								...
								const scalar dSq = tnbLib::magSqr(points[spI] - p);
	...
	) const
	{
		...
		while (iter++ < 40)
		{
			...
			if (tnbLib::magSqr(newP - nearest) < tol * distSq)
				break;
