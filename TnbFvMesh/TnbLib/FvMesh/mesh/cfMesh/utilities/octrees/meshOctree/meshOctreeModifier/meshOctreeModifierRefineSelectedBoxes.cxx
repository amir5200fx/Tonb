...
#include <meshOctreeCubeCoordinatesScalar.hxx>
...
# endif
		...
		label maxLevel = tnbLib::max(nLayers);
...
# endif
				forAll(activeLeaves, i)
					localMax = tnbLib::max(localMax, targetRefLevel[activeLeaves[i]]);

# ifdef USE_OMP
# pragma omp critical
# endif
				maxLevel = tnbLib::max(localMax, maxLevel);
			}
...
# pragma omp parallel
		{
			...
			{
				if (refineBox[leafI])
					localMax = tnbLib::max(localMax, leaves[leafI]->level());
			}

# pragma omp critical
			maxLevel = tnbLib::max(maxLevel, localMax);
		}
# else
		forAll(refineBox, leafI)
		{
			if (refineBox[leafI])
				maxLevel = tnbLib::max(maxLevel, leaves[leafI]->level());
		}
# endif
...
# pragma omp parallel
			{
				...
				forAll(activeLeaves, i)
				{
					...
					nLayersForActive[i] = tnbLib::max(ceil(refThickness[leafI] / cs), 1);

					localMaxNLayers =
						tnbLib::max(localMaxNLayers, nLayersForActive[i]);
				}

# pragma omp critical
				maxNLayers = tnbLib::max(maxNLayers, localMaxNLayers);
			}
# else
			forAll(activeLeaves, i)
			{
				...
				maxNLayers = tnbLib::max(maxNLayers, nLayersForActive[i]);
			}
# endif
