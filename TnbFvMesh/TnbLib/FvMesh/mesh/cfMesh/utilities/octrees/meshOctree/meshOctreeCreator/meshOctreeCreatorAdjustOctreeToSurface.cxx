...
#include <objectRefinementList.hxx>
...
# endif
						...
						{
							rThickness[leafI] =
								tnbLib::max(rThickness[leafI], refThickness[oI]);
...
# endif
				forAll(surf, triI)
				{
					...
					{
						triBB.min() = tnbLib::min(triBB.min(), points[tri[pI]]);
						triBB.max() = tnbLib::max(triBB.max(), points[tri[pI]]);
					}
...
# endif
					...
					if (refThickness[emI] > VSMALL)
					{
						...
						forAll(intersectedLeaves, i)
						{
							...
							rThickness[leafI] =
								tnbLib::max(rThickness[leafI], refThickness[emI]);
						}
					}
				}
			}
