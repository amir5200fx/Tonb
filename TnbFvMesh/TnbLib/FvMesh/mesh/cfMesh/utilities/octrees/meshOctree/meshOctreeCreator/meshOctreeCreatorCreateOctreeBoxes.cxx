...
namespace tnbLib
{
	...
	void meshOctreeCreator::setRootCubeSizeAndRefParameters()
	{
		...
		do
		{
			...
			const scalar lSize = size / tnbLib::pow(2, label(globalRefLevel_));

			if (lSize < (maxSize * (1.0 - SMALL)))
			{
				const scalar bbSize =
					0.5 * maxSize * tnbLib::pow(2, label(globalRefLevel_));
		...
		if (meshDictPtr_->found("boundaryCellSize"))
		{
			...
			do
			{
				...
				const scalar lSize = maxSize / tnbLib::pow(2, addLevel);
				...
				forAll(entries, dictI)
				{
					...
					else if (patchDict.found("cellSize"))
					{
						...
						do
						{
							...
							const scalar lSize = maxSize / tnbLib::pow(2, nLevel);
