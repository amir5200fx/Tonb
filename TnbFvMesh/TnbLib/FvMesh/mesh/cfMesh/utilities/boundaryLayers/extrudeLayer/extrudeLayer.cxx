...
#include <labelledPointScalar.hxx>
	...
	void extrudeLayer::movePoints()
	{
		...
		if (Pstream::parRun())
		{
					...
					forAllRow(pointFaces, pointI, pfI)
					{
						...
						if (faceI >= nOrigFaces_)
						{
							...
							if (thickness_ < 0.0)
							{
								...
								forAll(f, pI)
									d = tnbLib::min(d, tnbLib::mag(points[f[pI]] - c));

								d *= 0.4;
								lps.scalarValue() = tnbLib::min(lps.scalarValue(), d);
							}
							else
							{
			...
			forAll(receivedData, i)
			{
				...
				d = tnbLib::min(d, lps.scalarValue());
			}
				...
				forAllRow(pointFaces, pointI, pfI)
				{
					...
					if (faceI >= nOrigFaces_)
					{
						...
						if (thickness_ < 0.0)
						{
							...
							thickness = tnbLib::min(thickness, d);
						}
					}
				}
