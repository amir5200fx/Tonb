			...
			forAll(layerPatches, lpI)
			{
				...
				if (it != numLayersForPatch_.end())
				{
					...
					else
					{
						...
						maxNumLayers = tnbLib::max(maxNumLayers, it->second);
						hasLocalValue = true;
					}
				}
			}
...
# endif
			forAll(splitEdges_, seI)
			{
				...
				forAllRow(pointFaces, bpI, pfI)
				{
					...
					//- at this edge
					nLayers = tnbLib::max(nLayers, nLayersAtBndFace_[bfI]);
					...
					if (tIt != maxThicknessForPatch_.end())
					{
						...
						{
							thickness = tnbLib::min(thickness, tIt->second);
						}
						else
						{
			...
			forAll(splitEdges_, seI)
			{
				...
				if (thicknessRatio[seI] > (1. + SMALL))
				{
					firstThickness =
						magv /
						(
						(1 - tnbLib::pow(thicknessRatio[seI], nLayersAtEdge[seI]))
...
# endif
				...
				firstThickness =
					tnbLib::min
					(
						tnbLib::max(firstLayerThickness[seI], SMALL),
						firstThickness
					);
				...
				if (specialMode_)
				{
					...
					for (label i = 1; i < nLayersAtEdge[seI] - 1; ++i)
						t += firstThickness * tnbLib::pow(thicknessRatio[seI], i);
...
# endif
					...
					param += firstThickness * tnbLib::pow(thicknessRatio[seI], pI);
