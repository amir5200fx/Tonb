		...
		forAll(c, fI)
		{
			...
			else
			{
				forAllRow(facesFromFace_, c[fI], cfI)
				{
					...
					cellsFromCell[tnbLib::max(nLayers - 1 - cfI, 0)].append(cf);
				}
			}
		}
...
# endif
				...
				{
					//- k is const
					i = tnbLib::min(nI, maxI);
					j = tnbLib::min(nJ, maxJ);
					k = maxCoordinate ? maxK : 0;
				}
				else if (normalDirection == 1)
				{
					//- j is const
					i = tnbLib::min(nJ, maxI);
					j = maxCoordinate ? maxJ : 0;
					k = tnbLib::min(nI, maxK);
				}
				else if (normalDirection == 2)
				{
					//- i is const
					i = maxCoordinate ? maxI : 0;
					j = tnbLib::min(nI, maxJ);
					k = tnbLib::min(nJ, maxK);
				}
