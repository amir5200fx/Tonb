...
# endif
		forAll(nodesToMap, i)
		{
			...
			forAllRow(pFaces, bpI, pfI)
			{
				...
				mappingDistance[i] = tnbLib::max(mappingDistance[i], d);
			}
		...
		if (Pstream::parRun())
		{
			...
			forAll(receivedData, i)
			{
				...
				mappingDistance[nI] = tnbLib::max(mappingDistance[nI], ls.value());
			}
		}
	}
...
# endif
		forAll(nodesToMap, i)
		{
			...
			if (distSq > maxDist)
			{
				...
				const scalar f = tnbLib::sqrt(maxDist / distSq);
