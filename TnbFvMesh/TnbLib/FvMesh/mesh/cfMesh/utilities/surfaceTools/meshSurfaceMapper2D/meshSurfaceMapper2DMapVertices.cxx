	...
	) const
	{
		...
		forAll(edgesToMap, i)
		{
			...
			forAllRow(eFaces, beI, efI)
			{
				...
				mIter->second = tnbLib::max(mIter->second, d);
			}
		...
		if (Pstream::parRun())
		{
			...
			forAll(receivedData, i)
			{
				...
				std::map<label, scalar>::iterator mIter = mappingDistance.find(beI);
				mIter->second = tnbLib::max(mIter->second, ls.value());
			}
		}
	}
...
# pragma omp parallel for if( size > 1000 ) shared(parallelBndEdges) \
    schedule(dynamic, tnbLib::max(1, size / (3 * omp_get_max_threads())))
# endif
		forAll(edgesToMap, i)
		{
			const label beI = edgesToMap[i];
...
# endif
		forAll(edgesToMap, eI)
		{
			...
			if (distSq > mappingDistance[beI])
			{
				...
				mapPoint = tnbLib::sqrt(mappingDistance[beI] / distSq) * disp + p;
...
# ifdef USE_OMP
		const label size = edgesToMap.size();
# pragma omp parallel for if( size > 1000 ) shared(parallelBndEdges) \
    schedule(dynamic, tnbLib::max(50, size / (3 * omp_get_max_threads())))
# endif
