...
# endif
			forAll(bFaces, bfI)
			{
				...
				scalar d(0.0);
				forAll(bf, pI)
					d = tnbLib::max(d, tnbLib::mag(c - points[bf[pI]]));
				d = 2.0 * d + VSMALL;
		...
		forAll(pEdges, bpI)
		{
			...
			if (check)
			{
				...
				forAllRow(pEdges, bpI, peI)
				{
					...
					rSq = tnbLib::max(rSq, dSq);
				}

				rSq *= 2.0;
				const scalar r = tnbLib::sqrt(rSq);
...
# endif
			forAll(edgeType_, beI)
			{
				...
				forAll(c, fI)
				{
					...
					forAll(f, pI)
					{
						pMin = tnbLib::min(pMin, points[f[pI]]);
						pMax = tnbLib::max(pMax, points[f[pI]]);
					}
				}
