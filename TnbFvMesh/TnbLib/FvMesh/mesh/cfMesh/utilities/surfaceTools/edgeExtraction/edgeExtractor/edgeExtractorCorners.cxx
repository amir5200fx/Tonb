...
namespace tnbLib
{
	...
	bool edgeExtractor::findCornerCandidates()
	{
		...
		forAll(bFaces, bfI)
		{
			...
			forAll(bf, pI)
			{
				const scalar d = 2.0 * tnbLib::mag(c - points[bf[pI]]);
				const label bpI = bp[bf[pI]];

				searchRange[bpI] = tnbLib::max(d, searchRange[bpI]);
			}
		}
		...
		if (Pstream::parRun())
		{
			...
			forAll(receivedData, i)
			{
				...
				searchRange[bpI] =
					tnbLib::max(searchRange[bpI], receivedData[i].value());
			}
		}
		...
		forAll(bPoints, bpI)
		{
			...
			forAll(containedTriangles, i)
			{
				...
				forAll(tri, pI)
				{
					...
					const scalar d = tnbLib::magSqr(sp[spI] - points[bPoints[bpI]]);
...
# endif
			...
			forAllConstIter(labelHashSet, featureEdge, it)
			{
				...
				(
					tnbLib::min(patches[0], patches[1]),
					tnbLib::max(patches[0], patches[1])
				);
