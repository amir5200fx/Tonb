...
# ifdef DEBUGSmooth
		...
		scalar Vmina(VGREAT);
		forAll(tets_, tetI)
			Vmina = tnbLib::min(Vmina, tets_[tetI].mag(points_));
...
# endif
		...
		do
		{
			...
			const scalar determinant = tnbLib::det(gradGradF);
...
# ifdef DEBUGSmooth
		scalar Vmin(VGREAT);
		forAll(tets_, tetI)
			Vmin = tnbLib::min(Vmin, tets_[tetI].mag(points_));
