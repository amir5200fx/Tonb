...
# endif
			...
			Amin = tnbLib::min(Amin, Atri);
			LsqMax = tnbLib::max(LsqMax, LSqrTri);
		}
...
# endif
			...
			const scalar Astab = tnbLib::max(VSMALL, 0.5 * (Atri + stab));
			func += LSqrTri / Astab;
		}
	...
	) const
	{
		...
		forAll(trias_, triI)
		{
			...
			const scalar Astab = tnbLib::max(ROOTVSMALL, 0.5 * (Atri + stab));
...
# endif
	...
	scalar surfaceOptimizer::optimiseSteepestDescent(const scalar tol)
	{
		...
		const scalar avgEdge = tnbLib::mag(pMax_ - pMin_);
...
# endif
		...
		pMax_()
	{
		...
		forAll(trias_, triI)
		{
			...
			for (label i = 1; i < 3; ++i)
			{
				pMin_ = tnbLib::min(pMin_, pts_[tf[i]]);
				pMax_ = tnbLib::max(pMax_, pts_[tf[i]]);
			}
		}
	}
