	...
	) const
	{
		...
		const scalar alpha =
			...
			(
				...
				(
					-1.0,
					tnbLib::min(1.0, cosAlpha / (currThickness + VSMALL))
				)
			);
		...
		const scalar beta =
			...
			(
				...
				(
					-1.0,
					tnbLib::min(1.0, cosBeta / (currNeiThickness + VSMALL))
				)
			);
		...
		if ((alpha + beta) < M_PI)
		{
			...
			const scalar sinGamma = tnbLib::max(SMALL, tnbLib::sin(gamma));
			const scalar sinAlpha = tnbLib::max(SMALL, tnbLib::sin(alpha));
			const scalar sinBeta = tnbLib::max(SMALL, tnbLib::sin(beta));
	...
	) const
	{
		...
		forAll(c, fI)
		{
			...
			if (help::shareAnEdge(bf, f) && (f.which(he.start()) == -1))
			{
				...
				maxThickness =
					tnbLib::min(maxThickness, maxDist);
			}
		}
...
# endif
		forAll(hairEdges_, hairEdgeI)
		{
			...
			hairLength[hairEdgeI] = (tnbLib::mag(n) + VSMALL);
						...
						forAll(bf, pI)
						{
							...
							forAllRow(hairEdgesAtBndPoint_, bpJ, peJ)
							{
								...
								if (nhe == hairEdges_[heJ])
								{
									...
									maxThickness =
										tnbLib::min(maxThickness, edgeThickness);
								}
							}
						}
					}
