#include <findCellsIntersectingSurface.hxx>
...
# endif
		forAll(cells, cellI)
		{
			...
			forAll(c, fI)
			{
				...
				forAll(f, pI)
				{
					bb.min() = tnbLib::min(bb.min(), points[f[pI]]);
					bb.max() = tnbLib::max(bb.max(), points[f[pI]]);
				}
			}
			...
			forAllConstIter(labelHashSet, triangles, tIter)
			{
				...
				for (label i = 1; i < 3; ++i)
				{
					...
					obb.min() = tnbLib::min(obb.min(), v);
					obb.max() = tnbLib::max(obb.max(), v);
				}
