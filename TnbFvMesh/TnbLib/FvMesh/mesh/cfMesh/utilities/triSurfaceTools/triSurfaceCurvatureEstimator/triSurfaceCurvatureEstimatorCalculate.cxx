...
# endif
			forAll(pointEdges, pI)
			{
				...
				if (features.size() == 2)
				{
					...
					scalar cs = e1 & e2;
					cs = tnbLib::min(1.0, cs);
					cs = tnbLib::max(-1.0, cs);

					const scalar curv = tnbLib::acos(cs) / (0.5 * (d1 + d2 + VSMALL));
					edgePointCurvature_[pI] = tnbLib::mag(curv);
				}
			}
		}
	}
...
# endif
	...
	void triSurfaceCurvatureEstimator::calculateGaussianCurvature()
	{
		...
		forAll(surface_, triI)
		{
			...
			forAll(tri, pI)
			{
				...
				gaussianCurvature_[region][tri[pI]] -= tnbLib::acos(-1.0 * cs);
		...
		forAll(meanCurvature_, patchI)
		{
			...
			for(std::map<label, scalar>::iterator it=gc.begin();it!=gc.end();++it)
			{
				...
				forAll(pEdges, peI)
				{
					...
					{
						maxCurv = tnbLib::max(maxCurv, gc[ovI]);
						minCurv = tnbLib::min(minCurv, gc[ovI]);
						++nNei;
					}
				}
	...
	void triSurfaceCurvatureEstimator::calculateMeanCurvature()
	{
		...
		forAll(edges, eI)
		{
			...
			scalar sn = t & ev;
			sn = tnbLib::min(1.0, sn);
			sn = tnbLib::max(-1.0, sn);
			...
			{
				//- calculate the angle
				const scalar angle = tnbLib::atan2(sn, cs);
				Hf += angle * length;
			}
		...
		forAll(meanCurvature_, patchI)
		{
			...
			for(std::map<label, scalar>::iterator it=mc.begin();it!=mc.end();++it)
			{
				...
				forAll(pEdges, peI)
				{
					...
					{
						maxCurv = tnbLib::max(maxCurv, mc[ovI]);
						minCurv = tnbLib::min(minCurv, mc[ovI]);
						++nNei;
					}
				}
