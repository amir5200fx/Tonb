...
namespace tnbLib
{
	...
	namespace triSurfaceChecks
	{
		...
		)
		{
			...
			const scalar tol = tnbLib::cos(angleTol * M_PI / 180.0);
...
# endif
		...
		{
			bb.min() = tnbLib::min(surf.points());
			bb.max() = tnbLib::max(surf.points());
		}
...
# endif
			forAll(intersected, tI)
			{
				...
				for (label i = 1; i < 3; ++i)
				{
					bb.min() = tnbLib::min(bb.min(), pts[tri[i]]);
					bb.max() = tnbLib::max(bb.max(), pts[tri[i]]);
				}
...
# endif
		...
		)
		{
			...
			const scalar cosVal = tnbLib::cos(angleTol * M_PI / 180.0);
