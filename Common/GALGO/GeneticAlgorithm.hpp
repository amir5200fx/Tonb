...
public: 
   ...
   //Func<T> Objective;
    //std::vector<T> (*Objective)(const std::vector<T>&* myCustomClass);
   using ObjectiveType = std::function<std::vector<T>(const std::vector<T>&)>;
   ...
   // constructor
   template <int...N>
   GeneticAlgorithm(int popsize, int nbgen, bool output, const Parameter<T,N>&...args);
...
GeneticAlgorithm<T>::GeneticAlgorithm(int popsize, int nbgen, bool output, const Parameter<T,N>&...args)
{
...
void GeneticAlgorithm<T>::print() const
{
   ...
   std::string messageForCustomClass;
   if (nogen % genstep == 0) {
      std::cout << " Generation = " << std::setw(std::to_string(nbgen).size()) << nogen << " |";
      messageForCustomClass += " Generation = " + std::to_string(nogen) + " |";
      for (int i = 0; i < nbparam; ++i) {
	      std::cout << " X";
          messageForCustomClass += " X";
         if (nbparam > 1) {
            std::cout << std::to_string(i + 1);
            messageForCustomClass += std::to_string(i + 1);
         }
         std::cout << " = "  << std::setw(9) << std::fixed << std::setprecision(precision) << bestParam[i] << " |";
         messageForCustomClass += " = " + std::to_string(bestParam[i]) + " |";
	   }
      for (unsigned i = 0; i < bestResult.size(); ++i) {
	      std::cout << " F";
          messageForCustomClass += " F";
         if (bestResult.size() > 1) {
            std::cout << std::to_string(i + 1);
            messageForCustomClass += std::to_string(i + 1);
         }
         std::cout << "(x) = " << std::setw(12) << std::fixed << std::setprecision(precision) << bestResult[i];
         messageForCustomClass += "(x) = " + std::to_string(bestResult[i]);
         if (i < bestResult.size() - 1) {
            std::cout << " |";
            messageForCustomClass += " |";
         } else {
            std::cout << "\n";
         }
	   }
