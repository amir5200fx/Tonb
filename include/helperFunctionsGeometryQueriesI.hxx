#pragma once
#include <helperFunctionsTopologyManipulation.hxx>
#include <helperFunctionsGeometryQueries.hxx>
	...
	namespace help
	{
		...
		)
		{
			...
			forAll(f1, pI)
			{
				...
				const scalar dot = tnbLib::max(-1.0, tnbLib::min(nOwn & nNei, 1.0));

				if (vol > -VSMALL)
				{
					//- the angle is in the interval [Pi, 2Pi>
					const scalar ang = tnbLib::acos(dot);
		...
		)
		{
			...
			const scalar determinant = tnbLib::mag(tnbLib::det(mat));
		...
		)
		{
			...
			for (label dir = 0; dir < 3; ++dir)
			{
				...
				if (mag(vd) > (SMALL * d))
				{
					if (vd >= 0.0)
					{
						tMin = tnbLib::max(tMin, (pMin[dir] - sd) / vd);
						tMax = tnbLib::min(tMax, (pMax[dir] - sd) / vd);
					}
					else
					{
						tMin = tnbLib::max(tMin, (pMax[dir] - sd) / vd);
						tMax = tnbLib::min(tMax, (pMin[dir] - sd) / vd);
					}
				}
		...
		)
		{
					...
					)
			{
				...
				const scalar t0Min = tnbLib::min(t00, t01);
				const scalar t0Max = tnbLib::max(t00, t01);
				const scalar t1Min = tnbLib::min(t10, t11);
				const scalar t1Max = tnbLib::max(t10, t11);
