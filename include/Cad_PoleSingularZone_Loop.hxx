#pragma once
#ifndef _Cad_PoleSingularZone_Loop_Header
#define _Cad_PoleSingularZone_Loop_Header
...
namespace tnbLib
{
	...
	namespace cadLib
	{
		...
		struct PoleSingularZone_LoopBase
		{
			static TnbMesh_EXPORT const word typeName;
		};

		template<class SurfPln>
		class PoleSingularZone_Loop
			: public Cad_PoleSingularZone<SurfPln>
			, public PoleSingularZone_LoopBase
		{
		...
		public:
			...
			PoleSingularZone_Loop()
			{}

			// constructors [12/31/2021 Amir]

			explicit PoleSingularZone_Loop(const Standard_Integer theIndex)
				: base(theIndex)
			{}

			PoleSingularZone_Loop(const Standard_Integer theIndex, const std::shared_ptr<SurfPln>& thePlane)
				: base(theIndex, thePlane)
			{}

			PoleSingularZone_Loop(const Standard_Integer theIndex, std::shared_ptr<SurfPln>&& thePlane)
				: base(theIndex, std::move(thePlane))
			{}

			explicit PoleSingularZone_Loop(std::shared_ptr<SurfPln>&& thePlane)
				: base(std::move(thePlane))
			{}

			explicit PoleSingularZone_Loop(const std::shared_ptr<SurfPln>& thePlane)
				: base(thePlane)
			{}
...
#include <Cad_PoleSingularZone_LoopI.hxx>

#endif // !_Cad_PoleSingularZone_Loop_Header
