#pragma once
#ifndef _labelledPair_Header
#define _labelledPair_Header
...
namespace tnbLib
{
	...
	public:
		...
		inline bool operator<(const labelledPair& lpp) const
		{
			...
			if (
				tnbLib::min(pair_.first(), pair_.second()) <
				tnbLib::min(lpp.pair().first(), lpp.pair().second())
				)
				return true;
