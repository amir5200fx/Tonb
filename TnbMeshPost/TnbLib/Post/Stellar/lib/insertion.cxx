                  ...
                  int *listsize)
{
    ...
    memcpy(&list[*listsize], tet, sizeof(cavitytet));
    /* increment list size */
    (*listsize)++;
}
                      ...
                      cavityface *face)
{
    ...
    /* copy in the face */
    memcpy(&tet->outfaces[whichface], face, sizeof(cavityface));
}
                     ...
                     starreal *v)
{
    ...
    c[0] = ((vertex *) tetcomplextag2vertex(mesh, t[0]))->coord;
    c[1] = ((vertex *) tetcomplextag2vertex(mesh, t[1]))->coord;
    c[2] = ((vertex *) tetcomplextag2vertex(mesh, t[2]))->coord;
    c[3] = ((vertex *) tetcomplextag2vertex(mesh, t[3]))->coord;
                    ...
                    bool allowvertexdeletion)
{
    ...
    *cavitysize = 0;
    ...
    v = ((vertex *) tetcomplextag2vertex(mesh, vtag))->coord;
...
{
    if ( ((cavityedge *)a)->qual > ((cavityedge *)b)->qual) return 1;
    if ( ((cavityedge *)a)->qual < ((cavityedge *)b)->qual) return -1;
    return 0;
}
...
{
    if ( ((cavityedgeortet *)a)->qual > ((cavityedgeortet *)b)->qual) return 1;
    if ( ((cavityedgeortet *)a)->qual < ((cavityedgeortet *)b)->qual) return -1;
    return 0;
}
...
void sortedges(cavityedge array[],
               int arraylength)
{
    qsort(array, (size_t) arraylength, sizeof(cavityedge), compareedges);
}
...
void sortedgeortets(cavityedgeortet array[],
                    int arraylength)
{
    qsort(array, (size_t) arraylength, sizeof(cavityedgeortet), compareedgeortets);
}
                   ...
                   starreal *worstincavity)
{
    ...
    static cavityedgeortet edges[MAXCAVITYDAGEDGES]; /* list of all edges in the DAG */
    int numedges=0;
...
{
    int cavitysize;
    static cavitytet cavity[MAXCAVITYTETS];
#ifndef NO_TIMER
                       ...
                       starreal *shortest, starreal *longest)
{
    ...
    *shortest = 0.0;
    ...
    stackinit(&stack[0], sizeof(improvetet));
    stackinit(&stack[1], sizeof(improvetet));
    stackinit(&influencestack, sizeof(improvetet));
    ...
    for (i=0; i<numtets; i++)
    {
        ...
        stacktet = (improvetet *) stackpush(&stack[0]);
...
bool facetinsert(tetcomplex *mesh,
                 ...
                 int *numnewtets)
{
	vertex *newvertex;         /* pointer to newly allocated vertex */
    ...
    point[0] = ((vertex *) tetcomplextag2vertex(mesh, face[0]))->coord;
    point[1] = ((vertex *) tetcomplextag2vertex(mesh, face[1]))->coord;
    point[2] = ((vertex *) tetcomplextag2vertex(mesh, face[2]))->coord;
    
    /* fetch the ORIGINAL positions of the face vertices, because we want to start
       the vertex out on the original surface */
    quadrics[0] = (quadric *) arraypoolforcelookup(&surfacequadrics, face[0]);
    quadrics[1] = (quadric *) arraypoolforcelookup(&surfacequadrics, face[1]);
    quadrics[2] = (quadric *) arraypoolforcelookup(&surfacequadrics, face[2]);
                ...
                int *numnewtets)
{
    ...
    point[0] = ((vertex *) tetcomplextag2vertex(mesh, v1))->coord;
    point[1] = ((vertex *) tetcomplextag2vertex(mesh, v2))->coord;
    point[2] = ((vertex *) tetcomplextag2vertex(mesh, v3))->coord;
    point[3] = ((vertex *) tetcomplextag2vertex(mesh, v4))->coord;
    ...
    /* check that we got it right */
    assert(((vertextype *) arraypoolforcelookup(&vertexinfo, newtag))->kind == FREEVERTEX);
                   ...
                   bool boundedge)
{
    ...
    point[0] = ((vertex *) tetcomplextag2vertex(mesh, splitedge[0]))->coord;
    point[1] = ((vertex *) tetcomplextag2vertex(mesh, splitedge[1]))->coord;
    
    /* fetch the ORIGINAL positions of the edge vertices, because we want to start
       the vertex out on the original surface */
    quadrics[0] = (quadric *) arraypoolforcelookup(&surfacequadrics, splitedge[0]);
    quadrics[1] = (quadric *) arraypoolforcelookup(&surfacequadrics, splitedge[1]);
                  ...
                  bool tryhard)
{
    ...
    arraypoolstack cavitystack; /* save the tets affected by cavity improvement */
    arraypoolstack *cavitystackptr; /* may be null if outstack is null */
    ...
    stackinit(&cavitystack, sizeof(improvetet));
    if (outstack == NULL)
    {
        cavitystackptr = NULL;
    }
                ...
                bool quiet)
{
    ...
    while (tetstack->top != STACKEMPTY)
    {
        ...
        stacktet = (improvetet *) stackpop(tetstack);
