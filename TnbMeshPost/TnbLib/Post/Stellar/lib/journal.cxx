...
arraypoolstack* journal;
arraypoolstack journalstack;
...
{
    return ((journalentry *) arraypoolfastlookup(&(journal->pool), (unsigned long) journal->top))->id;
}
...
void copyjournaltop(arraypoolstack *fromstack,
                    ...
                    int fromtop)
{
	journalentry *fromentry, *toentry;
    int i;
    ...
    for (i=fromtop; i<=fromstack->top; i++)
    {
        ...
        fromentry = (journalentry *) arraypoolfastlookup(&(fromstack->pool), (unsigned long) i);
        toentry = (journalentry *) stackpush(tostack);
        
        /* copy it over */
        memcpy(toentry, fromentry, sizeof(journalentry));
    }
    ...
    if (improvebehave.verbosity > 5)
    {
        ...
        for (i=0; i<10; i++)
        {
            fromentry = (journalentry *) arraypoolfastlookup(&(fromstack->pool), (unsigned long) fromstack->top - i);
            toentry = (journalentry *) arraypoolfastlookup(&(tostack->pool), (unsigned long) tostack->top - i);
...
{
	arraypoolstack newjournal;
    stackinit(&newjournal, sizeof(journalentry));
    ...
    memcpy(journal, &newjournal, sizeof(arraypoolstack));
}
                        ...
                        starreal newpos[])
{
    ...
    entry = (journalentry *) stackpush(journal);    
    ...
    switch (type)
    {
        ...
        case INSERTVERTEX:
            ...
            ((vertextype *) arraypoolforcelookup(&vertexinfo, verts[0]))->kind = INPUTVERTEX;
            break;
        case CLASSIFY:
            entry->Class = LABEL;
                          ...
                          journalentry *entry)
{
    ...
    switch (entry->type)
    {
        case SMOOTHVERTEX:
            ...
            pos = ((vertex *) tetcomplextag2vertex(mesh, entry->verts[0]))->coord;
        ...
        case CLASSIFY:
            ...
            info = (vertextype *) arraypoolforcelookup(&vertexinfo, entry->verts[0]);
                        ...
                        journalentry *entry)
{
    ...
    switch (entry->type)
    {
        ...
        case INSERTVERTEX:
            ...
            ((vertextype *) arraypoolforcelookup(&vertexinfo, entry->verts[0]))->kind = DEADVERTEX;            
            /*
                      ...
                      int numentries)
{
    ...
    for (i=0; i<numentries; i++)
    {
        ...
        {
            entry = (journalentry *) stackpop(journal);
                       ...
                       int id)
{
    ...
    while(journal->top - i >= 0)
    {
        ...
        entry = (journalentry *) arraypoolfastlookup(&(journal->pool), (unsigned long) journal->top - i);
...
void printjournalrangestream(FILE *o, int startindex, int endindex)
{
    ...
    for (i=startindex+1; i<=endindex; i++)
    {
        ...
        {
            entry = (journalentry *) arraypoolfastlookup(&(journal->pool), (unsigned long) i);
                        ...
                        tag outtet[4])
{
    ...
    journalentry* entry = findjournalentryvertex(vtx, TOPOLOGICAL);
