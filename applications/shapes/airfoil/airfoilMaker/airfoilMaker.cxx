...
namespace tnbLib
{
	...
	)
	{
		Mesh_CurveOptmPoint_Correction<Geom2d_Curve, Geo2d_MetricPrcsr>
			correction(u0, guess, curve, *info.CorrAlgInfo());
		correction.Perform();
...
namespace tnbLib
{
	...
	{
		mod->add(chaiscript::fun([](const std::string& name)-> void {loadCurves(name); }), "loadProfiles");
...
int main(int argc, char *argv[])
{
	...
	if (argc IS_EQUAL 2)
	{
		...
		else if (IsEqualCommand(argv[1], "--run"))
		{
			...
			fileName myFileName("airfoilMaker");
