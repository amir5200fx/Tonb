...
namespace tnbLib
{
	...
	static const std::string loadExt = ".step";
	...
	void loadModel(const std::string& name)
	{
		...
		{// timer scope
			...
			myShape = std::make_shared<Cad_Shape>(0, file_path.filename().string(), Reader.OneShape());
		}
...
int main(int argc, char* argv[])
{
	...
	if (argc IS_EQUAL 2)
	{
		...
		{
			Info << " This application is aimed to read an STEP file format." << endl << endl;
			Info << endl
		...
		else if (IsEqualCommand(argv[1], "--run"))
		{
			...
			try
			{
				auto address = file::GetSystemFile("tnbStepReader");
