...
namespace tnbLib
{
	...
	void execute()
	{
		...
		auto [dx, dy, dz] = myShape->BoundingBox()->Length();
		Info << " dimensions: dx = "
			<< dx
			<< ", dy = " << dy
			<< ", dz = "
		...
		{
			++k;
			auto b = Cad_Tools::BoundingBox(Cad_Tools::BoundingBox(x));
			auto [dx, dy, dz] = b.Length();
			Info << " - solid no. " 
				<< k 
				<< ", dx = " << dx 
				<< ", dy = " << dy 
...
int main(int argc, char* argv[])
{
	...
	if (argc IS_EQUAL 2)
	{
		...
		{
			Info << endl;
			Info << " This application is aimed to show the dimensions of a shape." << endl << endl;
		...
		else if (IsEqualCommand(argv[1], "--run"))
		{
			...
			try
			{
				fileName myFileName(file::GetSystemFile("tnbCadToolShapeDim"));
