...
namespace tnbLib
{
	...
	static auto loadExt = Pln_Edge::extension;
	static auto saveExt = Pln_Edge::extension;
	...
	void loadFile(const std::string& name)
	{
		...
		myCurve = file::LoadFile<std::shared_ptr<Pln_Edge>>(name + loadExt, verbose);
	...
	void saveTo(const std::string& name)
	{
		...
		file::SaveTo(myCurve, name + saveExt, verbose);
	}
	...
	void execute()
	{
		...
		if (myRandAppx)
		{
			myCurve->Mesh() = myRandAppx->Chain();
		}

		if (myUniAppx)
		{
			myCurve->Mesh() = myUniAppx->Chain();
		}
...
int main(int argc, char* argv[])
{
	...
	if (argc IS_EQUAL 2)
	{
		...
		else if (IsEqualCommand(argv[1], "--run"))
		{
			...
			try
			{
				fileName myFileName(file::GetSystemFile("tnbCadToolPlaneDisctCurve"));
