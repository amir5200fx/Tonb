...
namespace tnbLib
{
	...
	void loadFile(const std::string& name)
	{
		...
		myMesh = 
			file::LoadFile<std::shared_ptr<Entity2d_QuadMesh>>
		(name + Entity2d_QuadMesh::extension, verbose);
		if(NOT myMesh)
		{
			FatalErrorIn(FunctionSIG)
				<< "the mesh file is empty." << endl
				<< abort(FatalError);
		}
		loadTag = true;
	}
...
int main(int argc, char* argv[])
{
	...
	if (argc IS_EQUAL 2)
	{
		...
		else if (IsEqualCommand(argv[1], "--run"))
		{
			...
			try
			{
				fileName myFileName(file::GetSystemFile("tnbQuadMesh2dToPlt"));
