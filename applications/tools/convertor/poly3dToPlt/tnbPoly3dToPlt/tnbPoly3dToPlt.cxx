...
namespace tnbLib
{
	...
	static const auto extension = Entity3d_Polygon::extension;

	static unsigned short verbose(0);
	static std::shared_ptr<Entity3d_Polygon> myPoly;
	...
	void loadFile(const std::string& name)
	{
		...
		myPoly = file::LoadFile<std::shared_ptr<Entity3d_Polygon>>(name + extension, verbose);
		if (NOT myPoly)
		{
...
int main(int argc, char* argv[])
{
	...
	if (argc IS_EQUAL 2)
	{
		...
		else if (IsEqualCommand(argv[1], "--run"))
		{
			...
			try
			{
				fileName myFileName(file::GetSystemFile("tnbPoly3dToPlt"));
