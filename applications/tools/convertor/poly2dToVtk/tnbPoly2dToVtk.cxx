...
namespace tnbLib
{
	...
	void saveTo(const std::string& name)
	{
		...
		fileName fn(name + ".vtk");
		OFstream myFile(fn);

		myPoly->ExportToVtk(myFile);
	}
...
int main(int argc, char* argv[])
{
	...
	if (argc IS_EQUAL 2)
	{
		...
		{
			Info << endl;
			Info << " This application is aimed to convert a polygon file to the *.vtk file format." << endl;
			Info << endl
		...
		else if (IsEqualCommand(argv[1], "--run"))
		{
			...
			try
			{
				fileName myFileName(file::GetSystemFile("tnbPoly2dToVtk"));
