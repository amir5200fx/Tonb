...
namespace tnbLib
{
	...
	auto retrieveCurves(const TopoDS_Shape& shape, double x)
	{
	...
	void execute()
	{
		...
		const auto shapes = retrieveShapeSections(*dist, myHull->Shape());
		myEdges.reserve(shapes.size());
		for (const auto& x : shapes)
		{
			auto edges = retrieveCurves(x.first, x.second);
			auto edges_ptr = std::make_shared<std::vector<std::shared_ptr<Pln_Edge>>>(std::move(edges));
			myEdges.push_back(std::move(edges_ptr));
		}

		exeTag = true;
		myBoundingBox = std::move(b);
	}
}
...
int main(int argc, char *argv[])
{
	...
	if (argc IS_EQUAL 2)
	{
		...
		else if (IsEqualCommand(argv[1], "--run"))
		{
			...
			fileName myFileName("hydstcHullSections");
