...
namespace tnbLib
{
	...
	static std::shared_ptr<marineLib::Body_Tank> myBody;
	...
	void loadModel(const std::string& name)
	{
		...
		myBody = std::dynamic_pointer_cast<marineLib::Body_Tank>(body);
		if (NOT myBody)
		{
			FatalErrorIn(FunctionSIG)
	...
	void execute()
	{
		...
		myModel = std::make_shared<marineLib::Model_Tank>(myBody->Index(), myBody->Name(), myBody);
	...
	if (argc IS_EQUAL 2)
	{
		...
		else if (IsEqualCommand(argv[1], "--run"))
		{
			...
			try
			{
				...
				//std::string address = ".\\system\\tnbHydstcTankModelMaker";
				fileName myFileName(file::GetSystemFile("tnbHydstcTankModelMaker"));
