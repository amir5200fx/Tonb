...
namespace tnbLib
{
	...
	static std::shared_ptr<marineLib::Body_Sail> myBody;
	...
	void loadModel(const std::string& name)
	{
		...
		myBody = std::dynamic_pointer_cast<marineLib::Body_Sail>(body);
		if (NOT myBody)
		{
			FatalErrorIn(FunctionSIG)
	...
	void execute()
	{
		...
		myModel = std::make_shared<marineLib::Model_BodySail>(myBody->Index(), myBody->Name(), myBody);
...
int main(int argc, char* argv[])
{
	...
	if (argc IS_EQUAL 2)
	{
		...
		{
			Info << endl;
			Info << " This application is aimed to create a model tank from a body." << endl;
			Info << endl
		...
		else if (IsEqualCommand(argv[1], "--run"))
		{
			...
			try
			{
				//std::string address = ".\\system\\tnbHydstcBodySailModelMaker";
				fileName myFileName(file::GetSystemFile("tnbHydstcBodySailModelMaker"));
