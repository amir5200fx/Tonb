...
namespace tnbLib
{
	...
	void loadModel(const std::string& name)
	{
		...
		if (NOT mySolutionData->IsUpdated(isc08Lib::SolutionData_Passenger::solutionOrder::displacerDim))
		{
			FatalErrorIn(FunctionSIG)
		...
		const auto& myModel = mySolutionData->Model();
		if (NOT myModel)
		{
			FatalErrorIn(FunctionSIG)
		...
		const auto& shape = myModel->Hull();
		if (NOT shape)
		{
			FatalErrorIn(FunctionSIG)
				<< "no shape of the hull has been found!" << endl
				<< abort(FatalError);
		}
		...
		const auto& displacer = model->Body();
	...
	void createFlatWave()
	{
		...
		mySolutionData->SetCurrentSolution(isc08Lib::SolutionData_Passenger::solutionOrder::wave);
	}
...
int main(int argc, char *argv[])
{
	...
	if (argc IS_EQUAL 2)
	{
		...
		else if (IsEqualCommand(argv[1], "--run"))
		{
			...
			fileName myFileName("isc08FlatWave");
