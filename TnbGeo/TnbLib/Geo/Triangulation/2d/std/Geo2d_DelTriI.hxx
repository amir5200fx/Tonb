...
namespace tnbLib
{
		...
		) const
	{
		...
		if (Geo_Tools::IsPointRightEdge_cgal(theCoord, Pts[v1], Pts[v2]))
			return TriLoc(theCoord, Neighb[Index_Of(i)].Value(0));
		else if (Geo_Tools::IsPointRightEdge_cgal(theCoord, Pts[v2], Pts[v3]))
			return TriLoc(theCoord, Neighb[Index_Of(i)].Value(1));
		else if (Geo_Tools::IsPointRightEdge_cgal(theCoord, Pts[v3], Pts[v1]))
