                        ...
                        &numboundfaces))
         {
             ...
             for (j=0; j<numboundfaces; j++)
             {
                     ...
                     (boundfacelist[j][2] == vtx))
                 {
                     ...
                     vc1 = ((vertex *) tetcomplextag2vertex(mesh, boundfacelist[j][0]))->coord;
                     vc2 = ((vertex *) tetcomplextag2vertex(mesh, boundfacelist[j][1]))->coord;
                     vc3 = ((vertex *) tetcomplextag2vertex(mesh, boundfacelist[j][2]))->coord;
                       ...
                       tag v4)
{
    ...
    for (i = 0; i < 4; i++) 
    {
        ...
        vptr = (vertextype *) arraypoollookup(&vertexinfo, tet[i]);
        assert(vptr != NULL);
        ...
        if (noghosts)
        {
            ...
            ((vertextype *) arraypoolforcelookup(&vertexinfo, tet[i]))->kind = FREEVERTEX;
        ...
        else
        {
            ...
            if (facetvertex(mesh, tet[i], incidenttettags, numincident, facetnormal))
            {
                ...
                info = (vertextype *) arraypoolforcelookup(&vertexinfo, tet[i]);
...
void classifyvertices(tetcomplex *mesh)
{
	...
	arraypoolstack localstack; /* local stack of tets that includes only one per vertex */
    ...
    stackinit(&localstack, sizeof(improvetet));
    ...
    while (tetstack.top != STACKEMPTY)
    {
        ...
        localtet = (improvetet *) stackpush(&localstack);
        /* copy tet over */
        ...
        for (i=0; i<4; i++)
        {
            ((vertextype *) arraypoolforcelookup(&vertexinfo, stacktet->verts[i]))->kind = INPUTVERTEX;
        }
    }
    ...
    {
        /* pull the top tet off the stack */
        stacktet = (improvetet *) stackpop(&localstack);
