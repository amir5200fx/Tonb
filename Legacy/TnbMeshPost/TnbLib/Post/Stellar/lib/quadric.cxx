                           ...
                           starreal v[3])
{
    ...
    q = (quadric *) arraypoolforcelookup(&surfacequadrics, vtag);
    assert(q != NULL);
...
{
	quadric *q = (struct quadric *) arraypoolforcelookup(&surfacequadrics, vtag);
    assert(q != NULL);
    return q->hasquadric;
}
                ...
                int numfaces)
{
    ...
    newv = (vertex *) proxipooltag2object(pool, vtag);
    ...
    for (i=0; i<numfaces; i++)
    {
        ...
        vptr[0] = (vertex *) proxipooltag2object(pool, faces[i][0]);
        vptr[1] = (vertex *) proxipooltag2object(pool, faces[i][1]);
        vptr[2] = (vertex *) proxipooltag2object(pool, faces[i][2]);
        
        /* get original vertex positions */
        vquads[0] = (quadric *) arraypoolforcelookup(&surfacequadrics, faces[i][0]);
        vquads[1] = (quadric *) arraypoolforcelookup(&surfacequadrics, faces[i][1]);
        vquads[2] = (quadric *) arraypoolforcelookup(&surfacequadrics, faces[i][2]);
...
void normalizequadrics(tetcomplex *mesh)
{
    ...
    while (vertextag != NOTATAG)
    {
        ...
        q = (quadric *) arraypoolforcelookup(&surfacequadrics, vertextag);
        assert(q != NULL);
...
void collectquadrics(tetcomplex *mesh)
{
    ...
    arraypoolinit(&surfacequadrics, sizeof(quadric), LOG2TETSPERSTACKBLOCK, 0);
    ...
    while (vertextag != NOTATAG)
    {
        ...
        ((quadric *) arraypoolforcelookup(&surfacequadrics, vertextag))->hasquadric = false;
    ...
    for (i=0; i<numfaces; i++)
    {
        ...
        /* get the actual vertices */
        vptr[0] = (vertex *) proxipooltag2object(pool, face[0]);
        vptr[1] = (vertex *) proxipooltag2object(pool, face[1]);
        vptr[2] = (vertex *) proxipooltag2object(pool, face[2]);
        ...
        */
        ...
        {
            /* fetch the quadric */
            q = (quadric *) arraypoolforcelookup(&surfacequadrics, face[j]);
            assert(q != NULL);
