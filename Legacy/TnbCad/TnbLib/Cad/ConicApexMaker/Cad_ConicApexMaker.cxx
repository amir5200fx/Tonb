...
//	}
//
//	auto CalcSurfNormals(const Dir3d& ax, const std::vector<Dir3d>& ns)
//	{
//		std::vector<Dir3d> normals;
//		normals.reserve(ns.size());
//
//		for (const auto& ni : ns)
//		{
//			auto n = ax.Crossed(ni);
//			normals.push_back(n);
//		}
//		return std::move(normals);
//	}
//
//	auto CalcSurfNormals(const std::vector<Cad_ConicApexMaker::Pw>& theBnds, const gp_Ax1& ax)
//	{
//		std::vector<Dir3d> normals;
//		try
//		{
//			normals.reserve(theBnds.size());
//
//			const auto Ox = Pnt3d(ax.Location());
//			const auto& dir = ax.Direction();
//
//			for (const auto& pw : theBnds)
//			{
//				const auto& p = RetrievePoint(pw);
//				Dir3d v(Ox, p);
//				auto n = dir.Crossed(v);
//				normals.push_back(n);
//			}		
//		}
...
//	}
//
//	auto CalcSurfNormals(const std::vector<Cad_ConicApexMaker::Pw>& theBnds0, const std::vector<Cad_ConicApexMaker::Pw>& theBnds1, const Pnt3d& theOx)
//	{
//		std::vector<Dir3d> normals;
//		try
//		{
//			normals.reserve(theBnds0.size());
//
//			for (size_t i = 0; i < theBnds0.size(); i++)
//			{
//				const auto& p0 = RetrievePoint(theBnds0[i]);
//				const auto& p1 = RetrievePoint(theBnds1[i]);
//
//				Dir3d v0(theOx, p0);
//				Dir3d v1(theOx, p1);
//
...
//
//			auto pnt = Pnt3d(pl->Value(intP.X(), intP.Y()));
//			return std::move(pnt);
//			/*Handle(Geom_Line) line = new Geom_Line(origin, dir);
//			auto projPt = ProjectPoint(pnt, line);
//			if (Distance(projPt, pnt) <= tol)
//			{
//				return std::move(pnt);
//			}
//			else
//			{
//				auto n = dir.Crossed(Dir3d(projPt, pnt));
//				auto pln = CreatePlane(pnt, n);
//				auto e2 = ProjectSegment(pnt, pl->Axis().Direction(), pln);
//				auto e3 = ProjectSegment(origin, dir, pln);
//
//				auto intP2 = Geo_Tools::IntersectionTwoLines(e2.P0(), CalcDir(e2), e3.P0(), CalcDir(e3), tol);
//
//				auto pnt2 = Pnt3d(pln->Value(intP2.X(), intP2.Y()));
//
//				return std::move(pnt2);
//			}*/
//		}
//		catch (const error& x)
//		{
...
//	const auto normSurfs = CalcSurfNormals(Axis().Direction(), normals);
//	//const auto normSurfs = CalcSurfNormals(B1, Axis());
//	//const auto normSurfs = CalcSurfNormals(B0, B1, origin);
//
...
//		//auto pln = CreatePlane(b1, normSurfs[i]);
//
