#include <codecvt>
#include <VoyageSim_MinFuel2.hxx>
...
std::map<Standard_Integer, std::shared_ptr<tnbLib::VoyageSim_Graph::Node>> tnbLib::VoyageSim_MinFuel2::
...
std::vector<std::shared_ptr<tnbLib::VoyageSim_Graph::Node>> tnbLib::VoyageSim_MinFuel2::Nexts(
...
tnbLib::VoyageSim_MinFuel2::CalcGraph
(
...
void tnbLib::VoyageSim_MinFuel2::Perform()
{
	static auto cmp =
		[](
	...
	while (true)
	{
		...
		for (const auto& [id, x]:nexts)
		{
			...
			if (auto iter = unvisited.find(next); iter NOT_EQUAL unvisited.end())
			{
				...
				auto tot_cost = current_value + cost.value;
				if (tot_cost < val)
				{
