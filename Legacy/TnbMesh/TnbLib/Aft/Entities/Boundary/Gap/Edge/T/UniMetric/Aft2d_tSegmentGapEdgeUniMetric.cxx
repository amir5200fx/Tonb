#include <Aft2d_tSegmentGapEdgeUniMetric.hxx>

#include <Aft2d_tCornerGapNodeUniMetric.hxx>
#include <Aft2d_tSegmentGapNodeUniMetric.hxx>
...
template<>
void tnbLib::Aft2d_tSegmentGapEdgeUniMetric::Contraction
(
...
)
{
	...
	/*if (auto n = std::dynamic_pointer_cast<Aft2d_tCornerGapNodeUniMetric>(this->Node0()))
	{
		n->BlowThisUp(thePrcsr);
	}
	else if (auto n = std::dynamic_pointer_cast<Aft2d_tCornerGapNodeUniMetric>(this->Node1()))
	{
		n->BlowThisUp(thePrcsr);
	}
	else
...
tnbLib::Aft2d_tSegmentGapEdgeUniMetric::GetTopology
(
...
)
{
	...
	auto n0 = std::make_shared<Aft2d_tCornerGapNodeUniMetric>(1, pts.at(0));
	Debug_Null_Pointer(n0);

	n0->InsertToCurves(theCurve->Index(), theCurve);

	auto n1 = std::make_shared<Aft2d_tSegmentGapNodeUniMetric>(2, pts.at(1));
	Debug_Null_Pointer(n1);
	...
	auto n2 = std::make_shared<Aft2d_tCornerGapNodeUniMetric>(3, pts.at(2));
	...
	{// create the first edge [4/26/2022 Amir]
		auto edge = std::make_shared<Aft2d_tSegmentGapEdgeUniMetric>(1, n0, n1);
		Debug_Null_Pointer(edge);
	...
	{// create the second edge [4/26/2022 Amir]
		auto edge = std::make_shared<Aft2d_tSegmentGapEdgeUniMetric>(2, n1, n2);
		Debug_Null_Pointer(edge);
