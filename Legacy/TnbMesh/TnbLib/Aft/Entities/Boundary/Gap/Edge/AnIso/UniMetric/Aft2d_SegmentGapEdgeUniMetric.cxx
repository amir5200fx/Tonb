#include <Aft2d_SegmentGapEdgeUniMetric.hxx>

#include <Aft2d_CornerGapNodeUniMetric.hxx>
#include <Aft2d_SegmentGapNodeUniMetric.hxx>
...
template<>
void tnbLib::Aft2d_SegmentGapEdgeUniMetric::Contraction
(
...
tnbLib::Aft2d_SegmentGapEdgeUniMetric::GetTopology
(
...
)
{
	...
	auto n0 = std::make_shared<Aft2d_CornerGapNodeUniMetric>(1, pts.at(0));
	Debug_Null_Pointer(n0);

	n0->InsertToCurves(theCurve->Index(), theCurve);

	auto n1 = std::make_shared<Aft2d_SegmentGapNodeUniMetric>(2, pts.at(1));
	Debug_Null_Pointer(n1);
	...
	auto n2 = std::make_shared<Aft2d_CornerGapNodeUniMetric>(3, pts.at(2));
	...
	{// create the first edge [4/26/2022 Amir]
		auto edge = std::make_shared<Aft2d_SegmentGapEdgeUniMetric>(1, n0, n1);
		Debug_Null_Pointer(edge);
	...
	{// create the second edge [4/26/2022 Amir]
		auto edge = std::make_shared<Aft2d_SegmentGapEdgeUniMetric>(2, n1, n2);
		Debug_Null_Pointer(edge);
