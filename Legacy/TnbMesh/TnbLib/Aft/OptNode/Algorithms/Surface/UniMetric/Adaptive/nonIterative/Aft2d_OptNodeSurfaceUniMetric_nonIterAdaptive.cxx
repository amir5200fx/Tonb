...
#include <Aft2d_CorrOptNodeSurfaceUniMetric_Fraction.hxx>
#include <TnbError.hxx>
#include <OSstream.hxx>

template<>
void tnbLib::Aft2d_OptNodeSurfaceUniMetric_nonIterAdaptive::Perform()
{
	Debug_Null_Pointer(this->GetInfo());
	Debug_Null_Pointer(GetInfo()->CalculatorInfo());
	Debug_Null_Pointer(GetInfo()->CorrectorInfo());

	auto alg = std::make_shared<Aft2d_IterOptNodeSurfaceUniMetric_Calculator>(GetInfo()->CalculatorInfo());
	...
	auto corrector =
		std::make_shared<Aft2d_CorrOptNodeSurfaceUniMetric_Fraction>
		(
			alg, MetricMap(),
			Front(), alg->Coord(),
			Size()
