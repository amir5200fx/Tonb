...
#include <Geo3d_PolygonRingGraphEdge.hxx>
#include <Geo3d_PolygonGraphNode.hxx>
#include <TnbError.hxx>
#include <OSstream.hxx>

namespace tnbLib
{
	std::pair<std::shared_ptr<Geo3d_PolygonGraphNode>, std::shared_ptr<Geo3d_PolygonGraphNode>>
		CreateNodes(const std::shared_ptr<Entity3d_Polygon>& thePoly)
	{
		...
		auto n0 = std::make_shared<Geo3d_PolygonGraphNode>(0, p0);
		auto n1 = std::make_shared<Geo3d_PolygonGraphNode>(1, p1);
		auto t = std::make_pair(std::move(n0), std::move(n1));
		return std::move(t);
	}

	std::shared_ptr<Geo3d_PolygonGraphNode>
		CreateRingNode(const std::shared_ptr<Entity3d_Polygon>& thePoly)
	{
		const auto& points = thePoly->Points();
		if (points.size() < 2)
		{
		...
		auto n = std::make_shared<Geo3d_PolygonGraphNode>(0, p0);
		return std::move(n);
	}
}

template<>
void tnbLib::Geo3d_KnitChain::Perform()
{
	theGraph_ = std::make_shared<Geo3d_PolygonGraph>();
	...
	while (start)
	{
		...
		auto edge = std::make_shared<Geo3d_PolygonSegmentGraphEdge>
			(++nbEdges, n0, n1);
	...
	if (NbEdges())
	{
		...
		while (start)
		{
			...
			auto edge = std::make_shared<Geo3d_PolygonRingGraphEdge>
				(++nbRings, n);
			Debug_Null_Pointer(edge);
