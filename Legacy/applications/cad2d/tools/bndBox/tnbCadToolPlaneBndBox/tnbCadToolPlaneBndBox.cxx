...
namespace tnbLib
{
	...
	void loadFile(const std::string& name)
	{
		...
		myShape = file::LoadFile<std::shared_ptr<Cad2d_Plane>>(name + loadExt, verbose);
		if (NOT myShape)
		{
	...
	void saveTo(const std::string& name)
	{
		...
		auto b = std::make_shared<Entity2d_Box>(myShape->BoundingBox(0));
...
int main(int argc, char *argv[])
{
	...
	if (argc IS_EQUAL 2)
	{
		...
		{
			Info << endl;
			Info << " This application is aimed to retrieve a bounding box of a plane." << endl;
			Info << endl
		...
		else if (IsEqualCommand(argv[1], "--run"))
		{
			...
			try
			{
				fileName theFileName(file::GetSystemFile("tnbCadToolPlaneBndBox"));
