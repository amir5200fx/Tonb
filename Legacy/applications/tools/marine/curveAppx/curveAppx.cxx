...
namespace tnbLib
{
	...
	void execute(const curve_t& myEdge)
	{
		const auto& myCurve = myEdge->Curve();
		if (randPnt)
		{
			Global_Timer timer;
...
int main(int argc, char *argv[])
{
	...
	if (argc IS_EQUAL 2)
	{
		...
		else if (IsEqualCommand(argv[1], "--run"))
		{
			...
			fileName myFileName("TnbMarineCurveAppx");
