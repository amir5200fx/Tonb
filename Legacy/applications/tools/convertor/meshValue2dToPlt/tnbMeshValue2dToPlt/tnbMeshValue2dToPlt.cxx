...
namespace tnbLib
{
	...
	static const std::string loadExt = Entity3d_MeshValue::extension;
	static const std::string saveExt = ".plt";

	static std::shared_ptr<Entity3d_MeshValue> myObj;
	...
	void loadFile(const std::string& name)
	{
		...
		myObj = file::LoadFile<std::shared_ptr<Entity3d_MeshValue>>(name + loadExt, verbose);
		if (NOT myObj)
		{
	...
	void saveTo(const std::string& name)
	{
		...
		const auto& tri = myObj->Mesh();
		const auto& values = myObj->Values();
	...
	if (argc IS_EQUAL 2)
	{
		...
		else if (IsEqualCommand(argv[1], "--run"))
		{
			...
			try
			{
				...
				fileName theFileName(file::GetSystemFile("tnbMeshValue2dToPlt"));
