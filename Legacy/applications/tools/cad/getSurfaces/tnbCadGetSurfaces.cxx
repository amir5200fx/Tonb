...
namespace tnbLib
{
	...
	static std::vector<std::shared_ptr<Cad_GeomSurface>> mySurfaces;
	...
	void saveTo(const std::string& name)
	{
		...
		file::SaveListTo<std::shared_ptr<Cad_GeomSurface>>(mySurfaces, name + saveExt, verbose);
	}
	...
	void execute()
	{
		...
		auto faces = Cad_Tools::RetrieveFaces(shape);
		mySurfaces.reserve(faces.size());
		for (const auto& x : faces)
		{
			auto g = BRep_Tool::Surface(x);
			if (g)
			{
				auto surf = std::make_shared<Cad_GeomSurface>(std::move(g));
				mySurfaces.push_back(std::move(surf));
			}
		}
...
int main(int argc, char *argv[])
{
	...
	if (argc IS_EQUAL 2)
	{
		...
		{
			Info << " This application is aimed to read a shape and retrieve the surfaces." << endl << endl;
			Info << endl
		...
		else if (IsEqualCommand(argv[1], "--run"))
		{
			...
			try
			{
				fileName theFileName(file::GetSystemFile("tnbCadGetSurfaces"));
