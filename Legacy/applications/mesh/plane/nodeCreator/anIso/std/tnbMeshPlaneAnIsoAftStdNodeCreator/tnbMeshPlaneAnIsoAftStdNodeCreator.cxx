...
namespace tnbLib
{
	...
	static const std::string extension = Aft2d_OptNodeAnIso_Calculator::extension;

	static unsigned short verbose = 0;
	static bool exeTag = false;

	static std::shared_ptr<Aft2d_OptNodeAnIso_Calculator> myNodeCreator;
	...
	void saveTo()
	{
		...
		saveTo(myNodeCreator->Name());
	}

	void execute(const std::string& name)
	{
		myNodeCreator = std::make_shared<Aft2d_StdOptNodeAnIso>();
		myNodeCreator->SetName(name);
...
int main(int argc, char *argv[])
{
	...
	if (argc IS_EQUAL 2)
	{
		...
		{
			Info << endl;
			Info << " This application is aimed to create an anisotropic node creator for the advancing front." << endl;
			Info << endl
		...
		else if (IsEqualCommand(argv[1], "--run"))
		{
			...
			try
			{
				fileName theFileName(file::GetSystemFile("tnbMeshPlaneAnIsoAftStdNodeCreator"));
