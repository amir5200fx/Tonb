...
namespace tnbLib
{
	...
	static const std::string extension = Aft2d_SolutionDataAnIso::extension;
	...
	auto loadGlobalInfo()
	{
		...
		auto name = file::GetSingleFile(boost::filesystem::current_path(), Aft_MetricPrcsrAnIso_Info::extension).string();

		auto metricInfo = file::LoadFile<std::shared_ptr<Aft_MetricPrcsrAnIso_Info>>(name + Aft_MetricPrcsrAnIso_Info::extension, verbose);
		if (NOT metricInfo)
		{
			FatalErrorIn(FunctionSIG)
	...
	auto loadMetricFunction()
	{
		checkFolder("metricMap");

		const auto currentPath = boost::filesystem::current_path();

		// change the current path [12/4/2021 Amir]
		boost::filesystem::current_path(currentPath.string() + R"(\metricMap)");

		auto name = file::GetSingleFile(boost::filesystem::current_path(), Geo2d_MetricFunction::extension).string();

		auto sizeFun = file::LoadFile<std::shared_ptr<Geo2d_MetricFunction>>(name + Geo2d_MetricFunction::extension, verbose);
		if (NOT sizeFun)
		{
			FatalErrorIn(FunctionSIG)
				<< " the metric function file is null" << endl
				<< abort(FatalError);
		}
	...
	auto loadNodeCreator()
	{
		...
		auto name = file::GetSingleFile(boost::filesystem::current_path(), Aft2d_OptNodeAnIso_Calculator::extension).string();

		auto alg = file::LoadFile<std::shared_ptr<Aft2d_OptNodeAnIso_Calculator>>(name + Aft2d_OptNodeAnIso_Calculator::extension, verbose);
		if (NOT alg)
		{
			FatalErrorIn(FunctionSIG)
	...
	void execute()
	{
		...
		auto metricFun = loadMetricFunction();

		auto plane = loadPlane();

		auto nodeCreator = loadNodeCreator();

		auto soluData = std::make_shared<Aft2d_SolutionDataAnIso>();
		...
		soluData->LoadMetricFunction(std::move(metricFun));
...
int main(int argc, char *argv[])
{
	...
	if (argc IS_EQUAL 2)
	{
		...
		{
			Info << endl;
			Info << " This application is aimed to create an aniso solution data of the plane mesh." << endl;
			Info << endl
		...
		else if (IsEqualCommand(argv[1], "--run"))
		{
			...
			try
			{
				fileName theFileName(file::GetSystemFile("tnbMeshPlaneSoluDataAnIsoAft"));
