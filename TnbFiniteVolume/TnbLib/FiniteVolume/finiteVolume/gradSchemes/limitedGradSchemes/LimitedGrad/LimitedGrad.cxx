	...
	namespace fv
	{
			...
			) const
		{
			...
			forAll(owner, faceI)
			{
				...
				// Owner side
				maxVf[own] = tnbLib::max(maxVf[own], vfIn[nei]);
				minVf[own] = tnbLib::min(minVf[own], vfIn[nei]);

				// Neighbour side
				maxVf[nei] = tnbLib::max(maxVf[nei], vfIn[own]);
				minVf[nei] = tnbLib::min(minVf[nei], vfIn[own]);
			}
			...
			forAll(bf, patchI)
			{
				...
				if (psf.coupled())
				{
					...
					forAll(pOwner, pFaceI)
					{
						...
						maxVf[own] = tnbLib::max(maxVf[own], psfNei[pFaceI]);
						minVf[own] = tnbLib::min(minVf[own], psfNei[pFaceI]);
					}
				}
				else
				{
					...
					forAll(pOwner, pFaceI)
					{
						...
						maxVf[own] = tnbLib::max(maxVf[own], vfNei);
						minVf[own] = tnbLib::min(minVf[own], vfNei);
					}
				}
			}
			...
			// HJ, 1/Feb/2016
			maxVf = tnbLib::max(maxVf - vf, pTraits<Type>::zero);
			minVf = tnbLib::min(minVf - vf, pTraits<Type>::zero);
