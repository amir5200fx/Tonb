#pragma once
#ifndef _Cad_LineSingularZone_Loop_Header
#define _Cad_LineSingularZone_Loop_Header
...
namespace tnbLib
{
	...
	namespace cadLib
	{
		...
		struct LineSingularZone_LoopBase
		{
			static TnbMesh_EXPORT const word typeName;
		};

		template<class SurfPln>
		class LineSingularZone_Loop
			: public Cad_LineSingularZone<SurfPln>
			, public LineSingularZone_LoopBase
		{
		...
		private:
			...
			std::shared_ptr<curveType> theHorizon3_;

		public:

			// default constructor [12/31/2021 Amir]

			LineSingularZone_Loop()
			{}


			// constructors [12/31/2021 Amir]

			explicit LineSingularZone_Loop(const Standard_Integer theIndex)
				: base(theIndex)
			{}

			LineSingularZone_Loop(const Standard_Integer theIndex, const std::shared_ptr<SurfPln>& thePlane)
				: base(theIndex, thePlane)
			{}

			LineSingularZone_Loop(const Standard_Integer theIndex, std::shared_ptr<SurfPln>&& thePlane)
				: base(theIndex, std::move(thePlane))
			{}

			explicit LineSingularZone_Loop(std::shared_ptr<SurfPln>&& thePlane)
				: base(std::move(thePlane))
			{}

			explicit LineSingularZone_Loop(const std::shared_ptr<SurfPln>& thePlane)
				: base(thePlane)
			{}
			...
			(
				...
				const std::shared_ptr<curveType>& theHorizon3
			)
			...
			void SetHorizon3(const std::shared_ptr<curveType>& theCurve)
			{
				theHorizon3_ = theCurve;
			}

			void SetHorizon3(std::shared_ptr<curveType>&& theCurve)
			{
				theHorizon3_ = std::move(theCurve);
			}
...
#include <Cad_LineSingularZone_LoopI.hxx>

#endif // !_Cad_LineSingularZone_Loop_Header
