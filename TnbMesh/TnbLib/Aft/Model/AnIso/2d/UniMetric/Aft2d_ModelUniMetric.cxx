...
unsigned short tnbLib::Aft2d_ModelUniMetric::verbose(0);

template<>
void tnbLib::Aft2d_ModelUniMetric::Import
(
...
namespace tnbLib
{
	...
	(
		...
		const Aft2d_NodeAnIso& theNode,
		const Aft2d_MetricPrcsrUniMetric& theMetric,
...
#ifdef AFT_MODEL_DEBUG
template<>
void tnbLib::Aft2d_ModelUniMetric::MeshingOneLevel()
{
	if (verbose)
	{
	...
	{
		std::vector<std::shared_ptr<Aft2d_NodeAnIso>>
			CandidateNodes,
			LocalNodes;

		std::vector<std::shared_ptr<Aft2d_EdgeAnIso>>
			Effectives,
			Local;
...
template<>
void tnbLib::Aft2d_ModelUniMetric::FindValidNode
(
	const std::vector<std::shared_ptr<Aft2d_NodeAnIso>>& theCandidates,
	const std::vector<std::shared_ptr<Aft2d_EdgeAnIso>>& theEffectives
)
{
	...
	for (const auto& x : theCandidates)
	{
		...
		if ((x NOT_EQUAL newNode) AND CheckAngle(current, node, *base::MetricMap(), 0.998))
		{
			flag = 3;
			continue;
		}
...
template<>
void tnbLib::Aft2d_ModelUniMetric::InsertNewFrontsToLevels()
{
#ifdef AFT_MODEL_DEBUG
...
template<>
void tnbLib::Aft2d_ModelUniMetric::CompactNumbering
(
...
template<>
void tnbLib::Aft2d_ModelUniMetric::ActiveFront
(
...
template<>
void tnbLib::Aft2d_ModelUniMetric::CheckBoundary
(
	const std::vector<std::shared_ptr<Aft2d_EdgeAnIso>>& theFronts
)
{
