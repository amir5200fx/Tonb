...
unsigned short tnbLib::Aft2d_tBoundaryOfPlaneSurfaceUniMetric::verbose(0);

template<>
void tnbLib::Aft2d_tBoundaryOfPlaneSurfaceUniMetric::RemoveDegeneracies()
{
	...
	for (const auto& x : boundaries)
	{
		...
		if (x->IsGap())
		{
			auto gapEdge = std::dynamic_pointer_cast<Aft2d_tSegmentGapEdgeUniMetric>(x);
...
template<>
void tnbLib::Aft2d_tBoundaryOfPlaneSurfaceUniMetric::UpdateFront()
{
	...
	std::vector<std::shared_ptr<Aft2d_NodeSurface>> nodes =
		Aft_Tools::RetrieveNodes(Aft2d_tSegmentEdgeUniMetric::UpCast(boundaries));
...
template<>
void tnbLib::Aft2d_tBoundaryOfPlaneSurfaceUniMetric::Perform()
{
	if (verbose)
	{
		tnbLib::Info << endl
	...
	std::vector<std::shared_ptr<Aft2d_tPlnWireSurfaceUniMetric>> wires;
	...
	for (const auto& wire : wires)
	{
		...
		for (const auto& x : curves)
		{
			...
			if (x->IsGap())
			{
				mesh = Cad_tPlnGapCurveUniMetric::TopoMesh<bndType>(x, MetricProcessor(), currentInfo);
			}
			else
			{
				mesh = Aft2d_tPlnCurveSurfaceUniMetric::TopoMesh<bndType>(x, MetricProcessor(), currentInfo);
			}
