#include <Aft2d_BoundaryOfPlaneUniMetric.hxx>
...
unsigned short tnbLib::Aft2d_BoundaryOfPlaneUniMetric::verbose(0);

#ifdef _DEBUG
template<>
void tnbLib::Aft2d_BoundaryOfPlaneUniMetric::UpdateFront()
{
...
template<>
void tnbLib::Aft2d_BoundaryOfPlaneUniMetric::RemoveDegeneracies()
{
	auto& boundaries = ChangeBoundaries();
	std::vector<std::shared_ptr<Aft2d_SegmentEdgeUniMetric>> modified;
	...
	for (const auto& x : boundaries)
	{
		...
		if (x->IsGap())
		{
			auto gapEdge = std::dynamic_pointer_cast<Aft2d_GapEdgeTemplate<Aft2d_SegmentEdgeUniMetric>>(x);
...
template<>
void tnbLib::Aft2d_BoundaryOfPlaneUniMetric::Perform()
{
	if (verbose)
	{
		tnbLib::Info << endl
	...
	std::vector<std::shared_ptr<typename Aft2d_RegionPlaneUniMetric::plnWireType>> wires;
	...
	for (const auto& wire : wires)
	{
		...
		std::vector<std::shared_ptr<Aft2d_SegmentEdgeUniMetric>> Medges;
		...
		for (const auto& x : curves)
		{
			...
			std::vector<std::shared_ptr<Aft2d_SegmentEdgeUniMetric>> mesh;
			if (x->IsGap())
			{
				mesh = Cad2d_PlnGapCurveUniMetric::TopoMesh<Aft2d_SegmentEdgeUniMetric>(x, MetricProcessor(), currentInfo);
			}
			else
			{
