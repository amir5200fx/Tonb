...
template<>
void tnbLib::Aft2d_gSegmentGapEdge::Contraction
(
...
)
{
	...
	else if (auto n = std::dynamic_pointer_cast<Aft2d_gCornerGapNode>(this->Node1()))
	{
		n->BlowThisUp(thePrcsr);
	}
	else
...
tnbLib::Aft2d_gSegmentGapEdge::GetTopology
(
...
)
{
	...
	auto n0 = std::make_shared<Aft2d_gCornerGapNode>(1, pts.at(0));
	Debug_Null_Pointer(n0);

	n0->InsertToCurves(theCurve->Index(), theCurve);

	auto n1 = std::make_shared<Aft2d_gSegmentGapNode>(2, pts.at(1));
	...
	auto n2 = std::make_shared<Aft2d_gCornerGapNode>(3, pts.at(2));
	...
	{// create the first edge [4/26/2022 Amir]
		auto edge = std::make_shared<Aft2d_gSegmentGapEdge>(1, n0, n1);
	...
	{// create the second edge [4/26/2022 Amir]
		auto edge = std::make_shared<Aft2d_gSegmentGapEdge>(2, n1, n2);
		Debug_Null_Pointer(edge);
