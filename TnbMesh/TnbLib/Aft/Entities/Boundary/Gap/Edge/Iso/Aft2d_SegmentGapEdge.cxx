#include <Aft2d_SegmentGapEdge.hxx>

#include <Aft2d_CornerGapNode.hxx>
#include <Aft2d_SegmentGapNode.hxx>
...
template<>
void tnbLib::Aft2d_SegmentGapEdge::Contraction
(
	const Geo2d_MetricPrcsr& thePrcsr
)
{
	auto n0 = std::dynamic_pointer_cast<Aft2d_BndNode>(this->Node0());
	auto n1 = std::dynamic_pointer_cast<Aft2d_BndNode>(this->Node1());
...
tnbLib::Aft2d_SegmentGapEdge::GetTopology
(
...
)
{
	...
	auto n0 = std::make_shared<Aft2d_CornerGapNode>(1, pts.at(0));
	Debug_Null_Pointer(n0);

	n0->InsertToCurves(theCurve->Index(), theCurve);

	auto n1 = std::make_shared<Aft2d_SegmentGapNode>(2, pts.at(1));
	Debug_Null_Pointer(n1);
	...
	auto n2 = std::make_shared<Aft2d_CornerGapNode>(3, pts.at(2));
	...
	{// create the first edge [4/26/2022 Amir]
		auto edge = std::make_shared<Aft2d_SegmentGapEdge>(1, n0, n1);
		Debug_Null_Pointer(edge);
	...
	{// create the second edge [4/26/2022 Amir]
		auto edge = std::make_shared<Aft2d_SegmentGapEdge>(2, n1, n2);
		Debug_Null_Pointer(edge);
