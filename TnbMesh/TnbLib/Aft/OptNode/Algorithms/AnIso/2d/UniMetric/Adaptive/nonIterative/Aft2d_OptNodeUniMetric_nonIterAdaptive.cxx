#include <Aft2d_OptNodeUniMetric_nonIterAdaptive.hxx>
...
#include <Aft2d_CorrOptNodeUniMetric_Fraction.hxx>
#include <TnbError.hxx>
#include <OSstream.hxx>

template<>
void tnbLib::Aft2d_OptNodeUniMetric_nonIterAdaptive::Perform()
{
	Debug_Null_Pointer(this->GetInfo());
	Debug_Null_Pointer(GetInfo()->CalculatorInfo());
	Debug_Null_Pointer(GetInfo()->CorrectorInfo());

	auto alg = std::make_shared<Aft2d_IterOptNodeUniMetric_Calculator>(GetInfo()->CalculatorInfo());
	...
	auto corrector =
		std::make_shared<Aft2d_CorrOptNodeUniMetric_Fraction>
		(
			alg, MetricMap(),
			Front(), alg->Coord(),
			Size()
