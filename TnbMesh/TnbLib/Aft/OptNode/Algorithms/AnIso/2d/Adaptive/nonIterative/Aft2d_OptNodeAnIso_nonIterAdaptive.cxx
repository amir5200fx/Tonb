#include <Aft2d_OptNodeAnIso_nonIterAdaptive.hxx>
...
#include <Aft2d_CorrOptNodeAnIso_Fraction.hxx>
#include <TnbError.hxx>
#include <OSstream.hxx>

template<>
void tnbLib::Aft2d_OptNodeAnIso_nonIterAdaptive::Perform()
{
	Debug_Null_Pointer(this->GetInfo());
	Debug_Null_Pointer(GetInfo()->CalculatorInfo());
	Debug_Null_Pointer(GetInfo()->CorrectorInfo());

	auto alg = std::make_shared<Aft2d_IterOptNodeAnIso_Calculator>(GetInfo()->CalculatorInfo());
	...
	auto corrector = 
		std::make_shared<Aft2d_CorrOptNodeAnIso_Fraction>
		(
			alg, MetricMap(), 
			Front(), alg->Coord(),
			Size()
