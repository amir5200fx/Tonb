#pragma once
#ifndef _Cad_LineSingularZone_TwoSide_Header
#define _Cad_LineSingularZone_TwoSide_Header
...
namespace tnbLib
{
	...
	namespace cadLib
	{
		...
		struct LineSingularZone_TwoSideBase
		{
			static TnbMesh_EXPORT const word typeName;
		};

		template<class SurfPln>
		class LineSingularZone_TwoSide
			: public Cad_LineSingularZone<SurfPln>
			, public LineSingularZone_TwoSideBase
		{
		...
		public:
			...
			LineSingularZone_TwoSide()
			{}

			// constructors [12/31/2021 Amir]

			explicit LineSingularZone_TwoSide(const Standard_Integer theIndex)
				: base(theIndex)
			{}

			LineSingularZone_TwoSide(const Standard_Integer theIndex, const std::shared_ptr<SurfPln>& thePlane)
				: base(theIndex, thePlane)
			{}

			LineSingularZone_TwoSide(const Standard_Integer theIndex, std::shared_ptr<SurfPln>&& thePlane)
				: base(theIndex, std::move(thePlane))
			{}

			explicit LineSingularZone_TwoSide(std::shared_ptr<SurfPln>&& thePlane)
				: base(std::move(thePlane))
			{}

			explicit LineSingularZone_TwoSide(const std::shared_ptr<SurfPln>& thePlane)
				: base(thePlane)
			{}
...
#include <Cad_LineSingularZone_TwoSideI.hxx>

#endif // !_Cad_LineSingularZone_TwoSide_Header
