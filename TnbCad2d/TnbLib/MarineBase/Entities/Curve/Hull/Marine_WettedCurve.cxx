...
tnbLib::Marine_WettedCurve::Copy() const
{
	...
	auto copy = std::make_shared<Marine_WettedCurve>(Index(), Name(), c);
...
<
	...
	std::shared_ptr<tnbLib::Pln_Curve>
>
tnbLib::Marine_WettedCurve::Split
(
...
) const
{
	...
	if (C0) curve0 = std::make_shared<Marine_WettedCurve>(C0);
	if (C1) curve1 = std::make_shared<Marine_WettedCurve>(C1);

	auto t = std::make_tuple(std::move(curve0), std::move(curve1));
	return std::move(t);
}

void tnbLib::Marine_WettedCurve::Split
(
	const Standard_Real x,
...
) const
{
	...
	theLeft = std::make_shared<Marine_WettedCurve>(C0);
	theRight = std::make_shared<Marine_WettedCurve>(C1);
}
...
) const
{
	...
	auto c2t = std::make_shared<Pln_TangCurve<Marine_WettedCurve>>(c2->Geometry());
